...
apt install -y software-properties-common apt-utils rsync mc htop dialog nginx curl && \
apt install -y php7.4-imagick php7.4-fpm php7.4-curl php7.4-xml php7.4-zip php7.4-mbstring php7.4-mysql && \
apt install -y mysql-server && \
echo "CREATE USER 'admin'@'%' IDENTIFIED WITH mysql_native_password BY 'yasooshae'; GRANT ALL PRIVILEGES ON *.* TO 'admin'@'%' WITH GRANT OPTION; FLUSH PRIVILEGES;" | mysql && \
